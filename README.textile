...
Allow for many event watchers, but trigger only one handler. This allows the watchers to signal if they handled the event or not, and if so terminate the event propigation.
